...
#include "HidMixer/BluefruitConnectionManagerDetail/central_profiles/BLECentralProfileHHKBStudio.h"
#include "bluefruit.h"

namespace HidMixer::BluefruitConnectionManagerDetail
{
  BLECentralProfileHHKBStudio::BLECentralProfileHHKBStudio(const char *path) : _path(path), _peer_addr(etl::nullopt)
  {
  }

  bool BLECentralProfileHHKBStudio::begin()
  {
    VERIFY(Dis.begin());
    VERIFY(Bas.begin());
    VERIFY(Hid.begin());
  ...
  bool BLECentralProfileHHKBStudio::canConnect(ble_gap_evt_adv_report_t *report)
  {
    ...
    else
    {
      ...
      {
        if (memcmp(buf, "HHKB-Studio", 11) == 0)
        {
          return true;
        }
      }
  ...
  bool BLECentralProfileHHKBStudio::discover(uint16_t conn_handle)
  {
  ...
  void BLECentralProfileHHKBStudio::successPairing(uint8_t peer_addr[6])
  {
    _peer_addr.emplace(peer_addr);
    AddrStore.save(_path, &_peer_addr.value(), sizeof(Addr));
  }

  void BLECentralProfileHHKBStudio::securedConnect()
  {
    Bas.enableNotify();
    Hid.enableKeyboard();
